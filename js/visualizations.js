...
async function generateTopicTrendsChart() {
  console.log('generateTopicTrendsChart called');
  ...
  console.log('Publications data available:', publicationsData.length, 'publications');
  ...
  console.log('hashColor function is available');
  ...
  console.log('Year-community map created:', Object.keys(yearCommunityMap).length, 'years');
...
document.addEventListener('DOMContentLoaded', () => {
  console.log('DOMContentLoaded - visualizations.js');
  ...
  if (targetNode) {
    console.log('Publications container found, setting up observer');
    const observer = new MutationObserver((mutations) => {
      if (mutations.some(mutation => mutation.type === 'childList' && mutation.addedNodes.length > 0)) {
        console.log('Publications rendered, initializing chart');
    ...
    // Also check if publications are already loaded (in case they loaded before observer was set up)
    const existingPublications = targetNode.querySelectorAll('.row.common-rows');
    if (existingPublications.length > 0) {
      console.log('Publications already present, initializing chart immediately');
      setTimeout(() => {
        generateTopicTrendsChart().catch(error => {
          console.error('Error generating topic trends chart:', error);
        });
    ...
    console.warn('Publications container not found, using fallback timeout');
    // Fallback to timeout if container not found
    setTimeout(() => {
      console.log('Fallback timeout - initializing chart');
      generateTopicTrendsChart().catch(error => {
        console.error('Error generating topic trends chart:', error);
      });
